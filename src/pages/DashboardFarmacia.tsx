...
function Avatar({ nombre, apellido }: { nombre: string; apellido: string }) {
    ...
    return (
        <div className={`${color} flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full text-sm font-bold text-white`}>
            {iniciales}
        </div>
    );
}
...
export default function DashboardFarmacia() {
    ...
    async function guardarEdicion(idStock: number) {
        const { error } = await supabase.from("stock").update({
            cantidad_disponible: parseInt(editCantidad),
            precio: parseFloat(editPrecio),
            ultima_actualizacion: new Date().toISOString(),
        }).eq("id_stock", idStock);
        if (!error) {
            setStock((prev: any[]) => prev.map((s) => s.id_stock === idStock ? { ...s, cantidad_disponible: parseInt(editCantidad), precio: parseFloat(editPrecio) } : s));
            setEditando(null);
        }
    }
    ...
    async function agregarMedicamento() {
        ...
        const { error } = await supabase.from("stock").insert({
            ...
            id_medicamento: parseInt(nuevoMed),
        ...
        if (!error) {
            setAgregando(false);
            setNuevoMed(""); setNuevaCantidad(""); setNuevoPrecio("");
            cargarStock();
        }
    }
    ...
    async function cargarEstadisticas() {
        ...
        if (todasReservas) {
            ...
            for (let i = 6; i >= 0; i--) {
                ...
                const key = d.toLocaleDateString("es-AR", { day: "2-digit", month: "2-digit", timeZone: "America/Argentina/Buenos_Aires" });
                dias[key] = 0;
            }
            todasReservas.forEach((r) => {
                const key = new Date(r.fecha).toLocaleDateString("es-AR", { day: "2-digit", month: "2-digit", timeZone: "America/Argentina/Buenos_Aires" });
        ...
        if (reservasEstados) {
            ...
            reservasEstados.forEach((r) => {
                const key = r.estado?.trim().toLowerCase();
            ...
            // Tiempo promedio: diferencia entre fecha de la reserva y ahora para las entregadas
            // Como no tenemos timestamp de cada cambio, usamos una estimación basada en días
            const entregadas = reservasEstados.filter(r => r.estado === "entregada");
            if (entregadas.length > 0) {
                const hoy = new Date();
                const promedioHoras = entregadas.reduce((acc, r) => {
                    const diff = hoy.getTime() - new Date(r.fecha).getTime();
                    return acc + diff / (1000 * 60 * 60);
                }, 0) / entregadas.length;
                // Mostramos en horas si < 48h, sino en días
    ...
    async function avanzarEstado(reserva: any) {
        ...
        // Si pasa a "entregada", marcar los receta_items como entregados
        if (siguiente === "entregada") {
            // Obtener los id_item de esta reserva
            const { data: reservaItems } = await supabase
                .from("reserva_items")
                .select("id_item")
            ...
            if (reservaItems && reservaItems.length > 0) {
                ...
                await supabase.from("receta_items")
                    .update({ entregado: true })
                    .in("id_item", idsItems);

                // Ver a qué recetas pertenecen esos items
                const { data: itemsData } = await supabase
                    .from("receta_items")
                    .select("id_receta, entregado")
                    .in("id_item", idsItems);

                if (itemsData) {
                    const idsRecetas = [...new Set(itemsData.map((i) => i.id_receta))];

                    // Para cada receta, verificar si todos sus items están entregados
                    for (const idReceta of idsRecetas) {
                        ...
                        if (todosItems && todosItems.every((i) => i.entregado)) {
    ...
    async function guardarNota(idReserva: number) {
        ...
        if (!error) {
            setReservas((prev) => prev.map((r) => r.id_reserva === idReserva ? { ...r, nota_interna: textoNota } : r));
            setNotaAbierta(null);
            setTextoNota("");
        }
    ...
    const reservasFiltradas = reservas
        ...
        .filter((r) => {
            ...
            return r.paciente?.nombre?.toLowerCase().includes(q) || r.paciente?.apellido?.toLowerCase().includes(q) || r.paciente?.dni?.toLowerCase().includes(q);
        });

    const contadores = {
        pendiente: reservas.filter((r) => r.estado === "pendiente").length,
        preparando: reservas.filter((r) => r.estado === "preparando").length,
        lista: reservas.filter((r) => r.estado === "lista").length,
        entregada: reservas.filter((r) => r.estado === "entregada").length,
        cancelada: reservas.filter((r) => r.estado === "cancelada").length,
    };

    const stockFiltrado = stock
        .filter((s) => mostrandoSoloBajos ? s.cantidad_disponible <= LIMITE_STOCK_BAJO : true)
        .filter((s) => busquedaStock === "" || s.medicamento?.nombre_generico?.toLowerCase().includes(busquedaStock.toLowerCase()));
            ...
            <header className="bg-white border-b-2 border-blue-600 shadow-sm">
                <div className="mx-auto flex max-w-7xl items-center justify-between px-8 py-3">
                    ...
                    <div className="text-right">
                        <p className="font-semibold text-slate-800">{nombreFarmacia}</p>
            ...
            <main className="mx-auto max-w-5xl p-8">
                <div className="mb-8 flex gap-3 flex-wrap">
                    {[{ key: "pedidos", label: "Pedidos recibidos" }, { key: "estadisticas", label: "Estadísticas" }, { key: "stock", label: "Mi Stock" }].map((tab) => (
                        <button key={tab.key} onClick={() => setVista(tab.key as Vista)} className={`rounded-xl px-6 py-3 font-semibold transition ${vista === tab.key ? "bg-blue-600 text-white shadow-md shadow-blue-200" : "bg-white text-slate-600 shadow-sm hover:bg-slate-50"}`}>
                            {tab.label}
                            {tab.key === "stock" && stockBajos.length > 0 && (
                                <span className="ml-2 rounded-full bg-red-500 px-2 py-0.5 text-xs text-white">{stockBajos.length}</span>
                            )}
                        </button>
                    ))}
                </div>
                ...
                {vista === "pedidos" && (
                    <>
                        <div className="mb-8 grid grid-cols-5 gap-3">
                            {ESTADOS.map((e) => (
                                <div key={e.key} className="rounded-2xl bg-white p-4 shadow-sm flex flex-col items-center gap-1 border border-slate-100">
                                    <p className="text-2xl font-bold text-slate-800">{contadores[e.key as keyof typeof contadores]}</p>
                                    <p className="text-xs text-slate-500 font-medium text-center leading-tight">{e.label}</p>
                                </div>
                            ))}
                        </div>

                        <div className="mb-4">
                            <input type="text" placeholder="Buscar por nombre o DNI..." value={busquedaPaciente} onChange={(e) => setBusquedaPaciente(e.target.value)} className="w-full rounded-xl border border-slate-200 p-3 text-sm focus:outline-none focus:border-blue-400" />
                        </div>

                        <div className="mb-6 flex gap-2 flex-wrap">
                            <button onClick={() => setFiltroEstado("todos")} className={`rounded-xl px-4 py-2 text-sm font-medium transition ${filtroEstado === "todos" ? "bg-blue-600 text-white" : "bg-white text-slate-600 shadow-sm hover:bg-slate-50"}`}>
                                Todos ({reservas.length})
                            </button>
                            {ESTADOS.map((e) => (
                                <button key={e.key} onClick={() => setFiltroEstado(e.key)} className={`rounded-xl px-4 py-2 text-sm font-medium transition ${filtroEstado === e.key ? "bg-blue-600 text-white" : "bg-white text-slate-600 shadow-sm hover:bg-slate-50"}`}>
                        ...
                        {loading ? <p className="text-slate-400">Cargando pedidos...</p> : reservasFiltradas.length === 0 ? (
                            <div className="rounded-2xl bg-white p-12 text-center shadow-sm border border-slate-100">
                                <p className="text-lg font-semibold text-slate-600">{filtroEstado === "todos" ? "No hay pedidos todavía" : `No hay pedidos "${getEstado(filtroEstado).label}"`}</p>
                            </div>
                        ) : (
                            <div className="space-y-3">
                                {reservasFiltradas.map((r) => {
                                    ...
                                    return (
                                        <div key={r.id_reserva} className="rounded-2xl bg-white p-5 shadow-sm border border-slate-100 hover:shadow-md transition">
                                            <div className="flex items-start gap-4">
                                                <Avatar nombre={r.paciente?.nombre ?? ""} apellido={r.paciente?.apellido ?? ""} />
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-start justify-between gap-2">
                                                        <div>
                                                            <p className="font-bold text-slate-800">{r.paciente?.nombre} {r.paciente?.apellido}</p>
                                                        ...
                                                        <div className="flex items-center gap-2 flex-shrink-0">
                                                            {r.estado === "cancelada" ? (
                                                                ...
                                                                <span className="rounded-full px-3 py-1 text-xs font-semibold bg-slate-100 text-slate-600">Entregada</span>
                                                            ) : (
                                                                <>
                                                                    {siguiente && (
                                                                        <button onClick={() => avanzarEstado(r)} className="rounded-xl bg-blue-600 px-3 py-1.5 text-xs text-white font-medium hover:bg-blue-700 transition">
                                                                            → {getEstado(siguiente).label}
                                                                        </button>
                                                                    )}
                                                                    <button onClick={() => cancelarReserva(r)} className="rounded-xl bg-red-50 px-3 py-1.5 text-xs text-red-600 font-medium hover:bg-red-100 transition">
                                                                        Cancelar
                                                                    </button>
                                                                </>
                                                            )}
                                                            ...
                                                            {ESTADOS.filter(e => e.key !== "cancelada").map((e, i) => {
                                                                ...
                                                                return (
                                                                    <div key={e.key} className="flex-1">
                                                                        <div className={`h-1.5 rounded-full transition-all ${activo ? "bg-blue-500" : "bg-slate-200"}`} />
                                                                        <p className={`mt-1 text-xs text-center ${esActual ? "text-blue-600 font-semibold" : activo ? "text-blue-400" : "text-slate-300"}`}>{e.label}</p>
                                                                    </div>
                                                                );
                                                            })}
                                                        </div>
                                                    ...
                                                    <div className="mt-3">
                                                        {r.nota_interna && !notaEstaAbierta && (
                                                            <p className="text-xs text-slate-500 bg-slate-50 rounded-lg px-3 py-2 border border-slate-200">
                                                                <span className="font-medium text-slate-600">Nota: </span>{r.nota_interna}
                                                            </p>
                                                        )}
                                                        {notaEstaAbierta ? (
                                                            <div className="flex gap-2 mt-1">
                                                                <input
                                                                    ...
                                                                    autoFocus
                                                                />
                                                                <button onClick={() => guardarNota(r.id_reserva)} disabled={guardandoNota} className="rounded-xl bg-blue-600 px-3 py-2 text-xs text-white hover:bg-blue-700 disabled:opacity-50">
                                                                    Guardar
                                                                </button>
                                                                <button onClick={() => { setNotaAbierta(null); setTextoNota(""); }} className="rounded-xl bg-slate-100 px-3 py-2 text-xs text-slate-600 hover:bg-slate-200">
                                                                    Cancelar
                                                                </button>
                                                            </div>
                                                        ) : (
                                                            <button
                                                                onClick={() => { setNotaAbierta(r.id_reserva); setTextoNota(r.nota_interna ?? ""); }}
                                                                className="mt-1 text-xs text-slate-400 hover:text-blue-500 transition"
                                                            >
                    ...
                    <div className="space-y-6">
                        {loadingStats ? <p className="text-slate-400">Cargando estadísticas...</p> : (
                            <>
                                {/* Tiempo promedio */}
                                {tiempoPromedio !== null && (
                                    <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100 flex items-center gap-6">
                                        <div>
                                            <p className="text-4xl font-bold text-blue-600">
                                                {tiempoPromedio < 48 ? `${tiempoPromedio}h` : `${Math.round(tiempoPromedio / 24)}d`}
                                            </p>
                                            <p className="text-sm font-medium text-slate-700 mt-1">Tiempo promedio por pedido</p>
                                            <p className="text-xs text-slate-400">Desde que se hace la reserva hasta que se entrega</p>
                                        </div>
                                        <div className="ml-auto text-right">
                                            <p className="text-sm text-slate-500">{contadores.entregada} pedido{contadores.entregada !== 1 ? "s" : ""} entregado{contadores.entregada !== 1 ? "s" : ""}</p>
                                        </div>
                                    </div>
                                )}

                                <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
                                    <h2 className="mb-1 text-base font-bold text-slate-800">Reservas últimos 7 días</h2>
                                    ...
                                    <ResponsiveContainer width="100%" height={200}>
                                        <LineChart data={reservasPorDia}>
                                            ...
                                            <Tooltip contentStyle={{ borderRadius: "12px", border: "none", boxShadow: "0 4px 24px #0001" }} />
                                ...
                                <div className="grid md:grid-cols-2 gap-6">
                                    <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
                                        <h2 className="mb-1 text-base font-bold text-slate-800">Estado de pedidos</h2>
                                        ...
                                        {estadosPie.length > 0 ? (
                                            <ResponsiveContainer width="100%" height={200}>
                                                <PieChart>
                                                    ...
                                                    <Legend iconSize={10} /><Tooltip contentStyle={{ borderRadius: "12px", border: "none" }} />
                                                </PieChart>
                                            </ResponsiveContainer>
                                    ...
                                    <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
                                        <h2 className="mb-1 text-base font-bold text-slate-800">Más buscados</h2>
                                ...
                                <div className="rounded-2xl bg-white p-6 shadow-sm border-l-4 border-red-400 border border-slate-100">
                                    <h2 className="mb-1 text-base font-bold text-slate-800">Oportunidades de stock</h2>
                    ...
                    <div>
                        {stockBajos.length > 0 && (
                            <div className="mb-6 rounded-2xl border-l-4 border-red-500 bg-red-50 p-4">
                                <p className="font-semibold text-red-700">{stockBajos.length} medicamento{stockBajos.length > 1 ? "s" : ""} con stock bajo (≤{LIMITE_STOCK_BAJO} unidades)</p>
                                <p className="mt-1 text-sm text-red-500">{stockBajos.map((s) => s.medicamento?.nombre_generico).join(", ")}</p>
                            </div>
                        )}
                        <div className="mb-4 flex gap-3 flex-wrap items-center">
                            <input type="text" placeholder="Buscar medicamento..." value={busquedaStock} onChange={(e) => setBusquedaStock(e.target.value)} className="flex-1 rounded-xl border border-slate-200 p-3 min-w-[200px] focus:outline-none focus:border-blue-400" />
                            <button onClick={() => setMostrandoSoloBajos(!mostrandoSoloBajos)} className={`rounded-xl px-4 py-3 text-sm font-medium transition ${mostrandoSoloBajos ? "bg-red-500 text-white" : "bg-white text-slate-600 shadow-sm hover:bg-slate-50 border border-slate-200"}`}>
                                {mostrandoSoloBajos ? "Ver todos" : `Ver solo bajos (${stockBajos.length})`}
                            </button>
                            <button onClick={() => setAgregando(true)} className="rounded-xl bg-blue-600 px-4 py-3 text-sm font-medium text-white hover:bg-blue-700 transition">
                                + Agregar medicamento
                            </button>
                        </div>
                        {agregando && (
                            <div className="mb-6 rounded-2xl bg-white p-5 shadow-sm border border-blue-200">
                                <h3 className="mb-3 font-semibold text-blue-700">Agregar medicamento al stock</h3>
                                <div className="flex gap-3 flex-wrap">
                                    <select value={nuevoMed} onChange={(e) => setNuevoMed(e.target.value)} className="flex-1 rounded-xl border border-slate-200 p-3 min-w-[200px] focus:outline-none focus:border-blue-400">
                                        <option value="">Seleccioná un medicamento</option>
                                        {medicamentosDisponibles.map((m) => (
                                            <option key={m.id_medicamento} value={m.id_medicamento}>{m.nombre_generico} — {m.nombre_comercial}</option>
                                        ))}
                                    </select>
                                    <input type="number" placeholder="Cantidad" value={nuevaCantidad} onChange={(e) => setNuevaCantidad(e.target.value)} className="w-28 rounded-xl border border-slate-200 p-3 focus:outline-none focus:border-blue-400" />
                                    <input type="number" placeholder="Precio $" value={nuevoPrecio} onChange={(e) => setNuevoPrecio(e.target.value)} className="w-32 rounded-xl border border-slate-200 p-3 focus:outline-none focus:border-blue-400" />
                                    ...
                                    <button onClick={() => setAgregando(false)} className="rounded-xl bg-slate-100 px-4 py-3 text-slate-600 font-medium hover:bg-slate-200">Cancelar</button>
                                </div>
                            </div>
                        )}
                        {loadingStock ? <p className="text-slate-400">Cargando stock...</p> : stockFiltrado.length === 0 ? (
                            ...
                            <div className="space-y-2">
                                {stockFiltrado.map((s) => {
                                    ...
                                    return (
                                        <div key={s.id_stock} className={"rounded-2xl bg-white p-4 shadow-sm flex items-center gap-4 border border-slate-100"}>
                                            <div className="flex-1">
                                                <p className="font-semibold text-slate-800">{s.medicamento?.nombre_generico}</p>
                                                <p className="text-sm text-slate-400">{s.medicamento?.nombre_comercial}</p>
                                            </div>
                                            {editandoEste ? (
                                                <>
                                                    <input type="number" value={editCantidad} onChange={(e) => setEditCantidad(e.target.value)} className="w-24 rounded-xl border border-slate-200 p-2 text-center focus:outline-none focus:border-blue-400" placeholder="Cantidad" />
                                                    <input type="number" value={editPrecio} onChange={(e) => setEditPrecio(e.target.value)} className="w-28 rounded-xl border border-slate-200 p-2 text-center focus:outline-none focus:border-blue-400" placeholder="Precio" />
                                                    ...
                                                    <button onClick={() => setEditando(null)} className="rounded-xl bg-slate-100 px-3 py-2 text-sm text-slate-600 hover:bg-slate-200">Cancelar</button>
                                                </>
                                            ) : (
                                                <>
                                                    ...
                                                    <div className="text-center w-28">
                                                        <p className="text-lg font-semibold text-slate-700">${Number(s.precio).toLocaleString("es-AR")}</p>
                                                        <p className="text-xs text-slate-400">precio</p>
                                                    </div>
                                                    <button onClick={() => { setEditando(s.id_stock); setEditCantidad(String(s.cantidad_disponible)); setEditPrecio(String(s.precio)); }} className="rounded-xl bg-blue-50 px-3 py-2 text-sm text-blue-700 hover:bg-blue-100 transition">Editar</button>
                                                    <button onClick={() => eliminarStock(s.id_stock)} className="rounded-xl bg-red-50 px-3 py-2 text-sm text-red-600 hover:bg-red-100 transition">Eliminar</button>
                                                </>
                                            )}
                                        </div>
                                    );
