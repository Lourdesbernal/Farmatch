...
export default function MisRecetas() {
    ...
    const toggleQr = (id: number, e: React.MouseEvent) => {
        e.stopPropagation();
        setQrVisible(prev => {
            const next = new Set(prev);
            next.has(id) ? next.delete(id) : next.add(id);
        ...
        <div className="min-h-screen bg-[#F8FAFC]">
            ...
            <header className="bg-white border-b-2 border-blue-600 shadow-sm sticky top-0 z-10">
                <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-lg font-semibold text-slate-800">Mis recetas</h1>
                        <p className="text-xs text-slate-500">
                            {activas.length} activa{activas.length !== 1 ? "s" : ""} · {completadas.length} completada{completadas.length !== 1 ? "s" : ""}
                        </p>
                    </div>
                </div>
            </header>
            ...
            <main className="max-w-2xl mx-auto px-4 py-6 space-y-6">
                {loading ? (
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-10 text-center">
                        ...
                        <p className="font-medium text-slate-600">Sin recetas aún</p>
                            ...
                            <section className="space-y-3">
                                ...
                                {activas.map((receta, i) => (
                                    <RecetaCard
                                        ...
                                        receta={receta}
                                        index={recetas.indexOf(receta) + 1}
                                        medico={getMedico(receta.id_medico)}
                                        items={getItems(receta.id_receta)}
                                        getMedicamento={getMedicamento}
                                        expandida={expandidas.has(receta.id_receta)}
                                        qrVisible={qrVisible.has(receta.id_receta)}
                                        onToggle={() => toggleExpandida(receta.id_receta)}
                                        onToggleQr={(e) => toggleQr(receta.id_receta, e)}
                                    />
                                ))}
                            </section>
                        )}
...
function RecetaCard({
    ...
    const vencimiento = fechaVencimiento.toLocaleDateString("es-AR", {
        day: "numeric", month: "short", year: "numeric",
    });

    const emision = new Date(receta.fecha_emision).toLocaleDateString("es-AR", {
        day: "numeric", month: "short", year: "numeric",
    });


    return (
        <div className={`bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden transition-opacity ${completada ? "opacity-80" : ""}`}>
            {/* Cabecera clickeable */}
            <button className="w-full text-left px-5 py-4" onClick={onToggle}>
                <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                        <div className={`p-2 rounded-xl shrink-0 ${completada ? "bg-green-50" : "bg-blue-50"}`}>
                            <FileText size={18} className={completada ? "text-green-600" : "text-blue-600"} />
                        </div>
                        <div className="min-w-0">
                            <p className="font-semibold text-slate-800">Receta #{index}</p>
                            {medico && (
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        ...
                        <ChevronDown
                            size={16}
                            className={`text-slate-400 transition-transform duration-200 ${expandida ? "rotate-180" : ""}`}
                        />
                    </div>
                </div>
                ...
                <div className="border-t border-slate-100">
                    ...
                    {medico && (
                        <div className="px-5 py-4 border-b border-slate-100">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="font-semibold text-slate-800">Dr/a. {medico.nombre} {medico.apellido}</p>
                                    <div className="flex items-center gap-2 text-xs text-slate-400 mt-0.5">
                                        {medico.especialidad && <span>{medico.especialidad}</span>}
                                        {medico.especialidad && <span>·</span>}
                    ...
                    <div className="px-5 py-4 border-b border-slate-100 flex gap-6">
                        <div>
                            ...
                            <p className="text-sm font-medium text-slate-700 mt-0.5">{emision}</p>
                        </div>
                        <div>
                            <p className="text-xs text-slate-400">Vencimiento</p>
                            <p className="text-sm font-medium text-slate-700 mt-0.5">{vencimiento}</p>
                        </div>
                    </div>
                        ...
                        <div className="space-y-2">
                            {items.map(item => {
                                ...
                                return (
                                    <div
                                        key={item.id_item}
                                        className={`rounded-xl border p-3 flex items-start gap-3 ${item.entregado
                                            ? "bg-slate-50 border-slate-100"
                                            : "bg-blue-50/60 border-blue-100"
                                            }`}
                                    >
                                        <div className={`p-1.5 rounded-lg shrink-0 mt-0.5 ${item.entregado ? "bg-slate-200" : "bg-blue-100"}`}>
                                            <Pill size={13} className={item.entregado ? "text-slate-500" : "text-blue-600"} />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-start justify-between gap-2">
                                                <div>
                                                    <p className={`text-sm font-semibold ${item.entregado ? "text-slate-500" : "text-slate-800"}`}>
                                                        {med?.nombre_generico ?? "Medicamento"}
                                                    </p>
                                                    {med?.nombre_comercial && (
                                                        <p className="text-xs text-slate-400">{med.nombre_comercial}</p>
                                                    )}
                                                </div>
                                                <span className={`shrink-0 text-[10px] font-semibold px-2 py-0.5 rounded-full ${item.entregado
                                            ...
                                            <p className="text-xs text-slate-500 mt-1">
                                                {item.cantidad} unid.{item.indicaciones ? ` · ${item.indicaciones}` : ""}
                                            </p>
                                        </div>
                                    </div>
                                );
                    ...
                    <div className="px-5 py-4">
                        ...
                        {qrVisible && (
                            <div className="mt-3 flex items-start gap-4">
                                <img
                                    src={`https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=farmatch-receta-${receta.id_receta}&color=1e40af&bgcolor=f0f9ff`}
                                    alt={`QR receta ${receta.id_receta}`}
                                    className="rounded-xl border border-blue-100 shadow-sm"
                                    width={120}
                                    height={120}
                                />
                                <div className="text-xs text-slate-400 space-y-1 pt-1">
                                    <p className="font-medium text-slate-600">Receta #{receta.id_receta}</p>
                                    <p>Presentá este código en la farmacia para retirar tus medicamentos.</p>
                                    <p className="text-slate-300">ID: farmatch-receta-{receta.id_receta}</p>
                                </div>
                            </div>
                        )}
                    </div>
